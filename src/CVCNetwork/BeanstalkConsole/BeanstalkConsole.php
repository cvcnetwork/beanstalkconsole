...
use CVCNetwork\BeanstalkConsole\BeanstalkInterface;

class BeanstalkConsole {
  public $interface;
  ...
  protected function __init() {
    ...
    $config = array('servers' => array('127.0.0.1:11300'));
  ...
  protected function _main() {
    if (!isset($_GET['server'])) {
      if (!isset($this->_globalVar['config']['servers'][0])) {
        ...
        header(sprintf('Location: ?server=%s', $this->_globalVar['config']['servers'][0]));
        exit();
      }
    }

    try {
      $this->interface = new \CVCNetwork\BeanstalkConsole\BeanstalkInterface($this->_globalVar['server']);
  ...
  protected function _actionKick() {
    ...
    header(
      sprintf('Location: ?server=%s&tube=%s', $this->_globalVar['server'],
